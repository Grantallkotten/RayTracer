...
bool Triangle::Collision(Ray *ray, CollisionInfo &collisionInfo) {
  ...
  if (glm::dot(ray->getDir(), ray->getInObject() ? normal : normal) >
      -EPSILON) { // If the scalarproduct betwen the ray and normal is positiv
    return false;
  }
  ...
  collisionInfo.point =
      ray->getOrig() + t * ray->getDir(); // Changes the value on the vec3
