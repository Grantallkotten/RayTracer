...
KDTree<Photon> PhotonMapper::Map(Scene scene) {
		...
		for(LightSource* l : scene.LightSources) {
            ...
            for (unsigned int i = 0; i < photons_per_object; i++) {
                ...
                if (Sphere* s = dynamic_cast<Sphere*>(o)) {
                    Ray* ray = new Ray(p, glm::normalize(p - randomPointIn2DPlane(planeCenter, v1, v2, s->getRadius())));
                    ray->castRay(&scene, 1);
                }
            }
		}
...
Photon PhotonMapper::castPhoton() {
...
glm::vec3 randomPointIn2DPlane(const glm::vec3& origin, const glm::vec3& v1, const glm::vec3& v2, float radius) {
    float u = glm::linearRand(-1.0f, 1.0f); // Random value for the x-coordinate within [0, 1]
    float v = glm::linearRand(-1.0f, 1.0f); // Random value for the y-coordinate within [0, 1]
