...
public:
  ...
  Scene(Camera c = Camera(glm::vec3(-1.0f, 0.0f, 0.0f), 800),
    ...
    PhotonMapper::Map(this, photon_map, 10);
    camera.render(this, photon_map);
    camera.writePPM();
  ...
  void addLight(LightSource *s) { LightSources.push_back(s); }

  void standardScene() {
    // add(Tetrahedron(glm::vec3(4.0f, -2.0f, -3.0f), 2.0f, 2.0f,
    // Material(diffusion, AQUA)));

    // add(Tetrahedron(glm::vec3(8.0f, -3.0f, 0.0f), 2.0f, 4.0f));

    // add(new Sphere(glm::vec3(7.0f, 3.0f, -2.0f), 2.0f, Material(specularity,
    // WHITE)));

    add(new Sphere(glm::vec3(6.0f, -3.0f, 1.0f), 1.0f,
                   Material(translucence, WHITE)));

    // add(new Sphere(glm::vec3(7.0f, -0.0f, -2.0f), 1.0f, Material(glossy,
    // AQUA)));
    ...
    // Roof lamp
    addLight(new LightSource(glm::vec3(1.0f, 0.5f, 4.9f),
